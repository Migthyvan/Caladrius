...
class ServicesView(APIView):
    ...
    def get(self, request):  # Correction: ajout du paramètre request manquant
        ...
        Liste tous les contacts (réservé aux administrateurs)
        """
        service = ContactService.objects.all()
        serializer = ContactServicesSerializer(service, many=True)
        return Response(serializer.data)
    ...
    def post(self, request):
        ...
        Crée un nouveau contact et gère l'inscription newsletter
        """
        serializer = ContactServicesSerializer(data=request.data)  # Ici: ContactServicesSerializer au lieu de ContactSerializer
